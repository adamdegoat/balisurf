// A shortboard, built from an outline: 6'2" (1.88m), a rounded-off point nose, squash tail, rocker, domed deck, rounded rails.
import * as THREE from 'three';

export function makeBoard() {
  const L = 1.88, W = 0.49, T = 0.062, NL = 90, NW = 24;
  ...
  const OUT = [[-.05, .6], [0, .66], [.08, .78], [.25, .93], [.45, 1], [.62, .96], [.78, .82], [.88, .64], [.94, .48], [.975, .31], [.992, .15], [1, .0], [1.01, -.1]];
  ...
  const rocker = (u) => 0.085 * Math.pow(Math.max(0, u - 0.6) / 0.4, 2.2) + 0.03 * Math.pow(Math.max(0, 0.18 - u) / 0.18, 2);
  ...
  for (const side of [1, -1]) {
    ...
    for (let i = 0; i <= NL; i++) {
      ...
      for (let j = 0; j <= NW; j++) {
        ...
        const pad = side > 0 && u > 0.05 && u < 0.3 && Math.abs(v) < 0.86, groove = 0.82 + 0.18 * (0.5 + 0.5 * Math.sin(u * 110));   // (soft grooves: finer than the mesh can show turned to blotches)
