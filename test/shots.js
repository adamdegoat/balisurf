...
export async function moment(mode, what, seed = 7) {
  ...
  const stop = { sit: (r) => r.state === 'LIE' && r.stateT > 2.5, sitwave: (r) => r.state === 'LIE' && g.incoming().t < 4, paddle: (r) => r.paddling && r.stateT > 1.5 && !r.onFace, pop: (r) => r.state === 'POP' && r.stateT > 0.2,
