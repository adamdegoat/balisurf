...
export async function moment(mode, what, seed = 7) {
  ...
  const stop = { sit: (r) => r.state === 'LIE' && r.stateT > 2.5, paddle: (r) => r.paddling && r.stateT > 1.5 && !r.onFace, pop: (r) => r.state === 'POP' && r.stateT > 0.2,
  ...
  try {
    for (let i = 0; i < 60 * 60; i++) {
      const r = g.rider;
      if (what === 'sit') { g.input.paddleBtn = false; g.input.test = 0; }
...
export function inside() { const g = G(); if (saved) { g.camera.position.copy(saved.p); g.camera.quaternion.copy(saved.q); } g.CUT.value = 0.16; g.surfer.traverse((o) => { if (o.name === 'head') o.scale.setScalar(0.001); }); }
